...
def main():
    ...
    print("Welcome to connect four\nHow many columns is your board")
    columns = int(input())
    ...
    print("How many rows is your board")
    rows = int(input())
...
##                    printboard(board, columns, rows)
                    ...
                    randominsertcoin(board,  player1turn, columns, rows)
##                    printboard(board, columns, rows)


                player1turn = not player1turn
        if checkforwinnerwhowon(board, columns, rows, requiredtowin) == 1:
            player1winscount += 1
        board = [['0' for y in range(columns)] for x in range(rows)]
    print("player1winscount: "+ str(player1winscount))
...
def randominsertcoin(board, player1turn, columns, rows):
    columnchoice = random.randrange(1, columns+1)
...
def checkforwinnerwhowon(board, columns, rows, requiredtowin):
    if requiredtowin == 3:
        for x in range(rows):
            for y in range(columns):
                if y+2<columns:
