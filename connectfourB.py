...
def main():
    ...
    t_columns = True
    while t_columns == True:
        print("How many columns is your board")
        try:
            columns = int(input())
            t_columns = False
        except:
...
##            print(table)
            ...
            if player1turn == True:
                board = b
                printboard(t, wn, board, columns, rows)
                print(heuristic(board, columns, rows))

            else:
                insertcoin(t, wn, board,  player1turn, columns, rows)
...
def printboard(t, wn, board, columns, rows): 
    t.speed()
    t.ht()
    for x in range(rows):
        for y in range(columns):
            ...
            t.dot(size=10)
        #print("\n")
def windowpos(x, y, wn):
 #           tess.goto(x, y)
            wn.title("Got click at coords {0}, {1}".format(x, y))
            return (int(x-30)/30)
      
def insertcoin(t, wn, board, player1turn, columns, rows):
    if player1turn == True:
        ...
        tess = turtle.Turtle()
        columnchoice = wn.onclick(windowpos(x,y,wn=wn))
        print(columnchoice)
        time.sleep(5)
